const fs = require('fs');
const path = require('path');

const dataFilePath = 'src/data/index.ts';
let content = fs.readFileSync(dataFilePath, 'utf8');

const lines = content.split('\n');
const newLines = [];
let insideGallery = false;

for (const line of lines) {
  if (line.includes('export const GALLERY: GalleryItem[] = [')) {
    insideGallery = true;
  }
  if (insideGallery && line.includes('];')) {
    insideGallery = false;
  }

  if (insideGallery) {
    const match = line.match(/imageUrl:\s*"([^"]+)"/);
    if (match) {
      const imageUrl = match[1];
      const localPath = path.join('public', imageUrl.replace(/^\//, ''));
      if (!fs.existsSync(localPath)) {
        console.log('Removing reference to missing file:', imageUrl);
        continue; // Skip this line because the file is deleted
      }
    }
  }
  newLines.push(line);
}

fs.writeFileSync(dataFilePath, newLines.join('\n'));
console.log('Cleanup complete.');
